...
class GameRun:
    def __init__(self):
        ...
        try:
            ...
            logger.info("Serial connection to Arduino established")
                ...
                try:
                    ...
                    if line:
                        ...
                        if len(parts) >= 2:
                            x = int(parts[0])
                            y = int(parts[1])
                            # Map joystick to directions, invert y as per user feedback
                            if x < 400:
                                self.game_state.direction = "l"
                            elif x > 600:
                                self.game_state.direction = "r"

                            if y < 400:  # Joystick up (low y) -> game down (inverted)
                                self.game_state.direction = "d"
                            elif y > 600:  # Joystick down (high y) -> game up (inverted)
